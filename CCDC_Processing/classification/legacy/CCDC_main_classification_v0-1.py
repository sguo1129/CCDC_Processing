...
def allCalc(FileDir, num_c, nbands, ntrees, task, ntasks, s,e):
	try:
		num_c = int(num_c)
		nbands = int(nbands)
		ntrees = int(ntrees)
		...
		for j in range(task,size_irows+1, ntasks):
...
def main():
   ...
   # define options
	...
	# elif not ops.gt_end:
		...
		allCalc(ops.in_Folder, ops.num_coefs, ops.num_bands, ops.ntrees, ops.task, ops.ntasks,ops.y_start,ops.y_end)  
